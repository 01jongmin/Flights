...
#[openapi(tag = "Airports")]
#[get("/id/<airport_id>")]
pub async fn get_airport_by_id(conn: MyDatabase, airport_id: String) -> Result<Json<Vec<Airport>>, Status> {
    let airport = conn.run( move |c| {
        let query = format!("
...
pub struct BfsRouteResult {
    ...
    pub distance: i64,
    #[sql_type="BigInt"]
    pub parent_airport_id: i64,
    #[sql_type="BigInt"]
    pub route_id: i64,
}


/// - Do not test for limit > 1 as OpenAPI is not built to support results exceeding a certain size
#[openapi(tag="Airports")]
